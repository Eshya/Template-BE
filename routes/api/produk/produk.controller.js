const Model = require('./produk.model')
...
exports.insert = async (req, res, next) => {
    const data = req.body;
    try {
        const findJenisDOC = await Model.findOne({merk: data.merk})
        if(findJenisDOC) return res.json({error: 1019, message: 'produk has been inserted'})
